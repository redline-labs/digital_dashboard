...
- **Configuration** is YAML via reflected structs (`REFLECT_STRUCT` in
  ...
  follow.
- **`libs/gsof` parses at compile time, and that is the point.** A wrong GNSS
  field offset yields a plausible latitude, not a crash, so every parser is
  `constexpr` and `gsof_test_records` asserts against real captures in
  `static_assert`s -- if it builds, the offsets are right. The captures come
  from receivers rather than being hand-written, because a vector authored from
  the same reading of the ICD as the parser agrees with the parser even where
  both are wrong. The records added for a BD992 (13, 14, 28, 48, 62, 70, 74, 91,
  92, 96) are the exception: they were validated on live hardware, but a GSOF
  capture is a position fix -- through the satellite azimuths and elevations
  against the timestamp, not only the position records -- so that capture is not
...
- **The MOTOTRBO handshake is shaped by five defects a capture could not
  ...
  message, selects unacknowledged delivery, and correlates replies on the
  transaction id rather than the opcode. Each was wrong once, and every one of
...
- **The radio is read-only apart from the channel.** `libs/mototrbo` builds no
  ...
  implemented. The one thing that moves is the channel, and the node's
  `allow_channel_change` defaults to false. Do not sweep that opcode table to
  "complete" the enum.
- **DBC tests own their DBCs.** `libs/dbc_parser/tests/dbcs/` holds four
  ...
  outside the field.
  Decoded signals already have their narrowest exact type -- float below
  `max|raw| + |offset/scale|` of 2^20, integer arithmetic for integral scaling,
  see `docs/libs/dbc_parser.md` -- so a cast to the type a field already has is
  noise, and a narrowing cast is worth a second look.
...
- Comments in this codebase explain *why*, especially where a past bug informed
  ...
  **Keep them short.** A comment earns its length by what a reader would
  otherwise have to reverse-engineer -- not by restating the code, and not by
  narrating every trap hit on the way to writing it. The same goes for commit
  messages: say what changed, why, and what was not verified, then stop.
