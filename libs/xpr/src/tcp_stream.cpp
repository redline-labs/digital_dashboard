...
int connectWithTimeout(const addrinfo& info, std::chrono::milliseconds timeout)
{
    ...
    {
        // Immediate success: a loopback peer, usually.
                                                      ...
                                                      std::chrono::milliseconds connectTimeout)
{
    ...
    for (const addrinfo* candidate = resolved; candidate != nullptr; candidate = candidate->ai_next)
    {
        ...
        // to 40 ms to every exchange for no benefit: XNL frames are small,
        // complete messages and there is nothing to batch them with.
        const int one = 1;
...
bool TcpStream::sendAll(std::span<const std::uint8_t> data)
{
    ...
    {
        // MSG_NOSIGNAL rather than a SIGPIPE handler: writing to a radio that
        // has gone away must return an error, not kill the process. The macOS
        // SDK defines this too, so there is no platform split.
...
ssize_t TcpStream::recvSome(std::span<std::uint8_t> out, unsigned timeoutMs)
{
    ...
    if (n == 0)
    {
        ...
        // timeout, because the caller must stop waiting and reconnect -- and
        // an XNL session does not survive its socket.
        return -1;
    }
    if (n < 0)
    {
