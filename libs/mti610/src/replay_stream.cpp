...
Result<std::unique_ptr<ReplayStream>> ReplayStream::open(const std::string& path, Options options)
{
    ...
    if (bytes.empty())
    {
        ...
        // end-of-stream, which looks exactly like a device that came up and
        // said nothing.
...
std::unique_ptr<ReplayStream> ReplayStream::fromBytes(std::vector<std::uint8_t> bytes,
                                                      Options options)
{
    return std::unique_ptr<ReplayStream>(new ReplayStream(std::move(bytes), options));
}
...
bool ReplayStream::sendAll(std::span<const std::uint8_t> data)
{
    ...
    // caller that writes a command during replay is not torn down by it -- the
    // reply simply never comes, and the caller's own timeout handles it, which
    // is the same path a device that stopped answering would take.
    (void)data;
    return mOpen;
}
...
ssize_t ReplayStream::recvSome(std::span<std::uint8_t> out, unsigned timeoutMs)
{
    ...
    if (mOffset >= mBytes.size())
    {
        if (!mOptions.loop)
        {
            // End of the capture, reported the same way a closed port is.
            mOpen = false;
            return -1;
        }
        mOffset = 0;
