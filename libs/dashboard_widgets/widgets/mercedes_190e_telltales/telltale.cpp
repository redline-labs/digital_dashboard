...
{
    // Edge-qualified, which is what keeps the paint rare: a stream delivering
    // the same answer at 100 Hz repaints nothing.
...
QColor Mercedes190ETelltale::backgroundColor() const
{
    return qt_helpers::toQColor(lit() ? _cfg.warning_color : _cfg.normal_color);
}

QColor Mercedes190ETelltale::iconColor() const
{
    return lit() ? kAssertedIcon : kNormalIcon;
}
...
void Mercedes190ETelltale::paintEvent(QPaintEvent* /*event*/)
{
    ...
    if (mSvgRenderer && mSvgRenderer->isValid()) {
        ...
        if (svgSize.isValid()) {
            ...
            tintPainter.fillRect(svgPixmap.rect(), iconColor());
            tintPainter.end();
