...
// reach the widget.
//
// The zenoh thread writes each evaluated value into a one-slot mailbox and
// wakes the shared DeliveryTicker; its next frame hands whatever is in every
// slot to its widget, all in one pass. A value that is overwritten before the
// frame is simply never shown, which is the correct thing to do with a stale
// reading.
//
  ...
  public:
        ...
        , ticker_{DeliveryTicker::instance()}
    {
        ...
        if (subscriber_->isValid())
        {
            ...
            // at most one event per frame across every subscription. The raw
            // pointer, not ticker_: a QPointer is not readable off its thread.
            DeliveryTicker* const ticker = ticker_.data();
            subscriber_->setResultCallback<T>([this, ticker](T value)
            {
                {
        ...
        // Registered even when the expression did not compile, so that a
        // binding which cannot ever deliver goes stale like one that has
        // stopped. A gauge reading zero because its expression is broken is
        // worse than one saying it has no data.
        ticker_->add(this);
    }

    ~ExpressionSubscription() override
    {
        if (ticker_)
        {
            ticker_->remove(this);
