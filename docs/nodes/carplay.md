...
| `pair-setup M3 proof REJECTED`, then `the AirPlay handshake failed; ending the session` | the setup password really is wrong, or a build before 2026-09-21 met a public key with a zero top byte (one session in 256). Either way the session is ended and retried; a rejection that recurs on every attempt is the password |
