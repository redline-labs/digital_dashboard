...
Why `tools/map_build` exists, what was measured when it was built, and the
places where the obvious implementation is wrong. How to run it is on
[map_build](../tools/map_build.html); the race-track verb has its own notes in
[Race tracks](tracks.html).
...
parsed by `map_rules` for the graph and are still not written to tiles; without
the last two there is no way to draw an overpass correctly.

So map matching, speed limits and routing could never have been served from it,
and the extractor had to be ours regardless. Given that, the tiler came too.

## One classification table

The tiler and the graph builder both call `map_rules::classify()`; neither has
rules of its own. One call decides both the drawn road and the routable road,
which makes "the route goes down a road that isn't drawn on the map" impossible
rather than merely unlikely. Two rule sets eventually disagree, and that
symptom costs a day before anyone suspects the cause.

`classify()` returns a struct, not one enum, because the sharing is not
symmetric: a way can be drawn and not routable, or routable and drawn at a
different zoom than its render class alone would give. The gate in front of both
is `map_rules::hasLabelTags()`, and it is discussed under the layer counts
below, because it is where a whole layer went missing without an error.

## Where the memory goes

Measured on the SoCal extract (a 637 MB PBF); the per-verb table is on the user
page. Memory is the binding resource, and the peak is in extraction rather than
in tiling. Sampled through a `tile` run, RSS climbs to 10.8 GB at 22 s, drops to
...
feature list are alive at the same time, and they have to be, because a feature
cannot be built without the coordinates it references.
...
Every drawn feature carries its source OSM way id, which is what lets a client
recolour the road it is on, or draw a route by highlighting features it already
has, rather than overlaying a full-precision polyline that visibly diverges from
the simplified tile geometry at low zoom.
...
`--stop-at` (default `0.95`) keeps one shared rank, and the query searches it
directly: a plain bidirectional Dijkstra over a few hundred thousand nodes
...
Present, it is validated against the graph: build time, counts, and a checksum
over the edges and the turn restrictions. That last part is not theoretical: two
graphs differing only by one banned turn have byte-identical edge arrays, and a
hierarchy built for one of them turns through the ban in the other. A mismatch
is refused, loudly, because an overlay pointed at the wrong graph does not
crash. Its shortcuts name edge indices that still exist and now mean other
roads, and the router answers quickly, confidently, and wrongly.
...
sampled pair and prints `WORST COST DIFFERENCE` if any cost differs.
...
`labels.cpp` already read (the list is on the user page). Every one was counted
against the archive tilemaker produced from the same PBF, at z14, the only
honest zoom, because it is the only one where neither build simplifies or
merges. Roads, buildings, water, landcover and waterways all land within a
couple of percent. The gaps that remain are all explained, and every one of them
is us being more complete:
...
| `park` | 5 012 | 16 574 | national parks and preserves are `type=boundary` relations. tilemaker assembles only `type=multipolygon`, so Joshua Tree, Mojave and Anza-Borrego are absent from its archive. |
...
reaches either key first files both as ground cover, and Southern California has
hundreds of thousands of pools. This one rule moved `water` from 23 114 features
to 77 736 against tilemaker's 78 090.
...
For a strongly concave shape, a crescent-shaped reservoir, the centroid falls on
the land inside the crescent and the label sits outside its own lake. It matters
for a few hundred features out of a hundred thousand; worth fixing when
