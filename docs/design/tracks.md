...
What the track source data turned out to be, how a centreline is derived from
it, and why the QA gate and the storage are shaped as they are. Building,
serving and checking the tileset is on [tracks](../tools/tracks.html).

## What the source data actually is

Not what it looks like. A file named `Monza.geojson` holds a single ring whose
traced length is 11 530 m against a published lap of 5 762: exactly twice. It is
not a centreline and not a racing line. It is the edge of the tarmac, traced all
the way round the outside and then all the way back round the inside, joined
into one ring.
...
`tools/map_build/track_geometry.h` has no sqlite, no capnp, no GeoJSON and no
mbtiles in it, so the awkward cases are built by hand in
`tests/test_track_geometry.cpp` against an annulus, whose answers are known in
closed form.
...
An open centreline's direction is arbitrary, because the two folds are
interchangeable, so nothing may assume index zero is the start of the course
rather than the finish.

## The QA gate

Two checks, both internal to the geometry:
...
What the published figure used to catch, a file holding several loops where the
derivation is self-consistently wrong by a factor of two, as
...
The verdict counts over all 994 are on the user page. Spot checks against
published lengths: Monza 5751 vs 5762, the Nordschleife 20710 vs 20741, Spa 6970
vs 6994, Laguna Seca 3584 vs 3600, and among the point-to-point courses Pikes
Peak 19 698 vs 19 915, Osnabrück 2311 vs 2318.

A rejected track still renders. The outline is what draws and it is fine
whatever happened to the centreline. `quality` says why there is no lap
distance, rather than leaving the absence to be guessed at.
...
The on-track test is distance from the centreline against the local half width,
not a crossing test against the two loops. The crossing test is the obvious
implementation and it is wrong here: these points sit essentially on the
outline, 1–10 m from the nearest boundary vertex and usually equidistant from
two of them, that is, on an edge. Even-odd parity for a point on an edge is a
coin flip, and it rejected 310 of 844 perfectly good gates.

{: .warning }
Whatever eventually asks "am I on this track" has the same trap waiting: a point
on the racing surface is inside the outer loop and outside the inner one, while
a point in the infield is inside both. A single point-in-polygon call against
the outline answers the wrong question, and against an edge it answers it at
random.
...
The detail service exists because tiles are lossy: simplified per zoom, clipped
to a tile. None of what a lap needs survives that, and reassembling a circuit
from four clipped tiles to measure against it is not a thing anybody should
attempt. `centerlineDistanceCm` and `gate.centerlineOffsetCm` are what any
future distance-along-lap consumer needs, indexed by distance rather than time,
...
eventually does is not blocked behind a re-ingest. `bd992_mock` uses the
geometry rather than the distances, and its `--check` reports the derived length
against the catalogue's own as a cross-check.

On the drawing side, `MapWidget` holds one `TileSource` per archive, each
learning its own zoom range from its own server replies and keeping its own
cache and backoff. That is what makes "no coverage here" and "that archive is
missing" distinguishable, and it costs one extra query batch per viewport, which
is nothing for a layer where nearly every tile comes back `notFound` and absence
...
**Accepted centrelines run about 0.6% short** of `length_m`, consistently.
Confirmed as a source difference rather than an error in the pairing: 927 of the
outlines are KML digitisations, straight-chorded between vertices, while
`length_m` follows the true curve from a separate catalogue. Closing it would
need a spline resample, and there is no smoother source in the data.

**23 outlines are corrupt upstream, and are not drawn.** They hold other layouts
at the same venue concatenated, and never the layout they are named after:
`Buenos_Aires_F` is every Buenos Aires layout except F, `Road_Atlanta` is the
Short and Combo layouts, `Monza_without_Chicane` is Monza and the Combo. That
self-exclusion is the diagnostic; a deliberate "all layouts here" export would
include its own.

Traced to the source vectors rather than to any step in this pipeline. The named
layout survives as a raster for 21 of the 23 and does not exist at all for two
(`Buenos_Aires_F`, `estering_Combo`), so there is no clean vector to fall back
to.

These are the one case where a rejected track is not rendered. Everywhere else
the outline is sound and only the centreline failed; here the outline is not the
track it names, so drawing it would overdraw two real siblings and hang the
wrong name on them. The catalogue row is kept with its verdict, so the track is
...
variant: Road Atlanta (4050 m, next best 2825 m), Miller Motorsports Park Full,
Bruntingthorpe Full, Heartland Park Topeka D.
...
shape exists nowhere in the source, not as a vector and not as an image. Nothing
...
the gate is Racelogic's coordinate, the outline is the KML. Castle Combe Western
Sprint is 23 m out, Anneau du Rhin 3.0 km 16 m. Not fixable here.
