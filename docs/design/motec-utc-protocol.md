---
title: MoTeC UTC protocol
parent: Design notes
---

# MoTeC UTC protocol

The reverse-engineered protocol behind [can_motec](../libs/can_motec.html),
what has been verified against hardware and how, and what is still unknown.
The user-facing half is in [can_bridge](../nodes/can_bridge.html).
...
That is the only part of the library that can be called confirmed by the
tests, and it is what they are built around: a vector written from the same
reading of the protocol as the parser agrees with the parser even where both
are wrong. Those captures are what established that the extended-identifier
bit lives in bit 31 of the identifier rather than in the flags byte, that the
data block carries no CRC of its own, and that a Tx acknowledgement's byte
count is not a block length.
...
Bytes past the DLC are junk, not padding. Real hardware leaves stale buffer
content there, and `to_can_frame()` drops them. The extended bit is in the
identifier, not the flags: a decoder looking in the flags byte reads a 29-bit
`0x1CF81BEC` as an 11-bit identifier and hands over a frame that is wrong in
the one way nothing downstream detects.
...
made the device transmit it as a 29-bit frame; a PCAN dongle on the same bus
reported exactly that. Decoding a standard frame from the low bits gave a
large, stable, entirely wrong identifier. Neither failure is visible without
a second device on the bus, which is why the table above is in
...
status `0x22`. Index 1 is refused with `0x40`, a different status, so 1 looks
like a slot that exists and is reserved rather than a value out of range. The
captured client used 3, and that is the default.

One filter per session, whatever its index. The second write is refused with
`0x22` whether it repeats the first index or names the other valid one. An
`Open` is what clears it. So this is not a bank of filters to populate; it is
one filter, written once, before the Rx subscribe. Pattern and mask are
accepted freely, including an all-ones mask.
...
happens, so the symptom is a bridge that goes deaf ten seconds after it
starts, which reads exactly like a bus that went quiet.

The command that keeps it open is `Version` (`0x0F`), sent periodically; the
reference client calls this its "MinReq keep-alive". The driver sends one
...
Frames straddle packet boundaries in both directions, and several small
frames arrive in one packet, so the reader has to reassemble rather than
assume one read is one frame.

The FT245BM is a fixed 64-byte FIFO part with no baud rate, no latency timer
and no flow control. The driver deliberately issues none of those control
requests; doing so is the first thing a driver written against the far more
common FT232 would do, and this device does not answer them. Nothing in
mainline Linux binds a driver to `0403:dcd8` (`ftdi_sio` ignores MoTeC's
product id unless told about it with a `new_id` write), so the dongle is
...
right and the request id is echoed. It refuses all service, and because `Open`
is refused too there is no way back in by the front door.
...
The payload matters: an `Open` on tag 0 with no payload is answered `0x23`
and the device stays latched.

`can_motec` does this automatically. `handshake()` recognises `0x21`, sends
the unlock and reopens, and the log says so when it happens.
...
- a full `USBDEVFS_RESET`, which re-enumerates in half a second and comes back still latched, because the FT245BM in front is only a FIFO bridge and resetting USB does not reset the microcontroller behind it
- writing `usb1-portN/disable`, which maps to `CLEAR_FEATURE(PORT_POWER)`; on an Intel Cannon Lake root hub the request is accepted and the device re-enumerates, but Vbus is not actually switched, a logical disconnect only, and the latch survives (a powered hub with real per-port power switching would be a different matter)
- a full host reboot, because the board keeps Vbus up across a warm boot, so the microcontroller never loses power
...
refused 1 994 of 2 000 frames, and killing a subscriber three times
mid-stream all left the device perfectly healthy. The trigger is still open,
which is exactly why the recovery is automatic rather than documented as a
manual procedure.
...
hardware. The `Set` command exists and its argument is shaped like
`0x40000000 | (value << 18)`, but which register selects the bus speed is
unknown, so `set_bitrate()` returns `Unsupported` rather than pretending, and
nothing in the protocol reports the rate back.
...
All of these fail loudly rather than being approximated, because every one of
...
| **Leaving the link idle** | The session times out after ~10 s of client silence; the driver sends a periodic `Version` to prevent it. |
| **Closing a session** | No close command is known, and the device latches. |

## What has been verified, and how

The UDP transport is how the backend was first brought up, against the
reference gateway from `motec-gw-sim`: that exercises the envelope, the CRC,
the session sequence, the record handling, transmit and the statistics,
everything except the FTDI framing and the stream reassembly, which the unit
tests cover directly.

The USB transport has since run against a real UTC (serial 56536) with a
PCAN-USB Pro FD on the same 1 Mbit/s bus, each acting as the other's oracle.
Confirmed on hardware: the FTDI status stripping and the stream reassembly
(the device answers `Version 7.2`, which cannot be read without both being
right); the session sequence of Open, Version, Filter, Rx subscribe; receive,
including the keep-alive cadence that the stall detector uses; transmit, in
both identifier formats, checked by a second dongle on the wire; and the
identifier layouts and filter rules above, which is where the bugs were.
...
7 000/s is essentially the ceiling: a DLC 8 standard frame at 1 Mbit/s is
about 135 bits with stuffing, so the bus tops out near 7 400/s, and the
highest rate actually observed arriving was 7 920/s. The pipeline is lossless
right up to it, and there is no degradation over a 20-second soak.

Do not offer frames faster than the bus can carry them. Handing the adapter
15 000/s when the bus does 7 400/s loses roughly half of them: USB accepts
every transfer, `send()` returns success, and the adapter discards silently.
There is no transmit-overflow report in the protocol (`PCAN_UFD_MSG_OVERRUN`
is specifically a receive overflow). `Statistics::txFrames` therefore counts
...
The PCAN backend does not behave this way. It delivers a record at a time,
and a queue one entry deep still did not overflow at 2 000 frames/s, so a
depth that is fine there is not necessarily fine here. The 8 192 default
holds about two seconds of a saturated bus and is the right choice; anything
under about a hundred is asking for drops.

Forced overflow, 2 000 frames at 1 000/s into a 16-deep queue: `rxFrames
2040`, `rxDropped 1456`, and exactly 584 frames published, so `rxFrames -
rxDropped` matches what came out and the accounting is right; the oldest are
discarded and the survivors stay in order, spread across the whole run rather
than truncated at one end; and the channel recovers completely, with five
frames sent afterwards all arriving.
...
anything to do with CAN error conditions on the UTC, since the protocol
reports none, so there is nothing to check.
...
`send()` counts the frame optimistically because it cannot wait for a USB
round trip; the acknowledgement arrives on the receive thread and corrects
`txFrames`/`txDropped` there. Verified exactly: with 7 refusals the partner
received precisely `txFrames` frames.

## Two dongles on one bus, and what a silent channel means

Every measurement above used a PCAN and a UTC on the same bus as each other's
oracle, and that arrangement is what found both identifier-layout bugs:
neither is visible from one device. It also gives the diagnostic for the most
common failure. A UTC whose stored bit rate does not match the bus carries
nothing and reports no error, because the protocol has no way to read the
rate back and no error counters to show the mismatch. A second adapter on the
bus that does see traffic is the fastest way to tell "wrong timing" from "quiet
bus".

## Still unknown

Worth looking for in any future capture: the meaning of the four-byte Rx
subscribe payload (`FF FF FF 01` and `FF FF 64 01` have both been seen), the
RTR bit, the upper four bits of the flags byte, the `Set` register map, the
third dword of the Filter command, and what distinguishes filter index 2 from
index 3.
