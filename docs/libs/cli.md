...
`waitForInterrupt(tick, period)` is a node's main loop: it calls `tick` every
`period` (a second by default) and returns as soon as a signal arrives. The
handler also writes a byte to a pipe the wait sleeps on in `poll()`. That makes
the wake independent of which thread the kernel delivers the signal to, where
